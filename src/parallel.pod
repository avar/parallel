...
Print the BibTeX entry for GNU B<parallel> and silence citation notice
for future invocations by creating a F<~/.parallel/will-cite>
file. It's also possible to disable this on a system-wide basis by
creating F</etc/parallel/will-cite>.

If you'd like to disable the citation notice only for a single
invocation you can use B<--will-cite>.
